    ...
    getAssetsPackage = {
      ...
      pkgs.requireFile {
        ...
        message =
          ...
          '')
          + "\nDOWNLOAD USING DEPOTDOWNLOADER: You may use \"depotdownloader -app ${builtins.toString metadata.steam_app_id} -depot ${builtins.toString metadata.steam_windows_depot_id} -manifest ${versionInfo.steam_windows_manifest_id}${lib.optionalString (!(self.isPublicVersion versionInfo)) " -beta ${builtins.head versionInfo.steam_branches}"}\" with your Steam credentials (\"-username foobar\"). Remove the temporary .DepotDownloader folder with \"rm -r ./depots/${builtins.toString metadata.steam_windows_depot_id}/${builtins.toString versionInfo.steam_buildid}/.DepotDownloader\" then add the game files to the Nix store using \"nix store add-path --name deltarune ./depots/${builtins.toString metadata.steam_windows_depot_id}/${builtins.toString versionInfo.steam_buildid}\""
