    ...
    getAssetsPackage = {
      ...
      pkgs.requireFile {
        ...
        message =
          ...
          '')
          + "\nDOWNLOAD USING DEPOTDOWNLOADER: You may use \"depotdownloader -app ${builtins.toString metadata.steam_app_id} -depot ${builtins.toString metadata.steam_windows_depot_id} -manifest ${versionInfo.steam_windows_manifest_id}${lib.optionalString (!(self.isPublicVersion versionInfo)) " -beta ${builtins.head versionInfo.steam_branches}"}\" with your Steam credentials (\"-username foobar\"). Add game files to Nix store using \"nix store add-path --name deltarune ./depots/${builtins.toString metadata.steam_windows_depot_id}/${builtins.toString versionInfo.steam_buildid}\""
          + "\n\nRetry this command once the above steps are done. If this error reoccurs, check https://github.com/MichailiK/deltarune-nix#download-deltarune for troubleshooting steps.";
      };
  }))
