...
let
  ...
  mkDeltarunePkgsOfVersion =
    ...
    let
      ...
      supportedRuntimeNames = pkgs.lib.filter (
        runtimeName: runtimeSupportsConfiguration { inherit runtimeName versionInfo pkgs; }
      ) runtimes;
      # Find the first runtime that supports this system & appears first in the runtimes list
      preferredRuntimeName = pkgs.lib.findFirst (
        runtimeName: builtins.elem runtimeName supportedRuntimeNames
      ) null runtimes;
      ...
      runtimesPkgs = pkgs.lib.pipe supportedRuntimeNames [
        (builtins.map (runtimeName: {
          name = runtimeName;
          value = mkDeltarunePkgsWithRuntime {
            inherit
              runtimeName
              versionInfo
              assets
    ...
    (pkgs.lib.optionalAttrs (builtins.length (builtins.attrNames runtimesPkgs) > 1) (
    ...
    // (if preferredRuntimeName == null then { } else runtimesPkgs.${preferredRuntimeName});
in
forAllSystems (
  pkgs:
      ...
      pkgs.lib.pipe versionInfo [
        ...
        (pkgs.lib.mapAttrs' (
          ...
          let
            ...
            optionalDash =
              if (builtins.stringLength newName != 0) && !(pkgs.lib.hasPrefix "+" newName) then "-" else "";
